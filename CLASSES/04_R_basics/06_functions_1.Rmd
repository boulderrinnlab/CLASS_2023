...
# Now let's source this function and run 
...
# let's make a path to our util as we will use this over and over again
basepath <- "/scratch/Shares/rinnclass/CLASS_2023/<Your_folder>"
filepath <- "CLASS_2023/util/my_class_functions.R"
# pasting the two together so it can be read by source with file.path
utilpath <- file.path(basepath,filepath)
# Let's source our FUN
source(utilpath)

# now type fun(7,6) in console
# cool we have a function sourced for our environment to use anytime.
...
# to fix this we need to set y as a local variable in a funciton
...
# let's test it:
test <- import_peaks(consensus_file_path = peak_path)
...
# setting up the function: @param is path to peaks

import_peaks <- function(consensus_file_path = broadpeakfilepath) {
  # same as before getting a list of filepaths 
  peak_files <- list.files(consensus_file_path, full.names = T, pattern = ".broadPeak")
  
  # extracting DBP name from peak files
  dbp_name <- sapply(peak_files, function(x){
    y <-  str_extract(x, "([^\\/]+$)")
    unlist(strsplit(y, "_"))[[1]] })
...
my_peaks <- import_peaks(broadpeakfilepath)
...
Here is how we will use this moving forward (or what start of session would include)
