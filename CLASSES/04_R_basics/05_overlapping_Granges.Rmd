...
consensus_list <- lapply(dbps, consensus_from_reduced, p_list = peaks_list)
...
consensus_from_reduced <- function(dbp, p_list) {
  dbp_peaks <- p_list[grepl(as.character(dbp), names(p_list))]
  ...
  for(k in 1:length(dbp_peaks)) {
    suppressWarnings(peak_exists[,k] <- as.numeric(countOverlaps(all_peaks, dbp_peaks[[k]]) > 0))
  }
...
# prom_ov <- lapply(consensus_list, function(consensus) subsetByOverlaps(consensus, promoters))

lapply(dbps, function(dbp) rtracklayer::export(consensus_list[[dbp]], con = paste0("results/", dbp, "_consensus.bed")))


# # import each peak file into a compiled Granges
# peak_list <- c()
# for(i in 1:length(peaks_fl)) {
#   # Import peak files
#   peaks <- rtracklayer::import(peaks_fl[i])
...
#   names(peak_list)[length(peak_list)] <- dbp[i]
# }


# make a list of unique DBP names that we can use to merge peaks
dbp_df <- data.frame(table(dbp)) %>%  # data.frame(table(tf_name))
  filter(Freq > 1)
unique_tf <- as.character(dbp_df$dbp)


consensus_peaks <- list()
# What do we want to know...
# a) number of peaks in each replicate
# b) how many peaks overlapped in every replicate
peak_info <- tibble(dbp = character(), 
                    file = character(), 
                    npeaks = integer(), 
                    nconsensus = integer())
for(i in 1:nrow(dbp_df)) {
  
  # load all the peak files corresponding to this DBP[i] in unique_tf.
  # tf <- unique_tf[1] -- allows us to look at output
  dbp <- as.character(dbp_df$dbp[i])
  print(dbp)
  # indexing unique DBP name to file path (e.g., first 8 are CTCF files)
  dbp_index <- grep(dbp, peaks_fl)
  
  # takes the TF name and grabs the index in fl for those replicates
  dbp_files <- peaks_fl[dbp_index]
  
  # now make a list of GRanges in a peak_list using another for loop
  # READ_PEAKS being used 
  
  # We need to start with some peak set if we want to subsetByOverlaps
  replicate_peaks <- rtracklayer::import(dbp_files[[1]])
  dbp_consensus <- replicate_peaks
  
  # Collect some info about this replicate file
  peak_info <- bind_rows(peak_info,
                         tibble(dbp = dbp,
                                file = dbp_files[[1]],
                                npeaks = length(replicate_peaks)))
  # Now we can move to the next files..
  
  # 1. read in all peaks
  # dbp_peaks <- sapply(dbp_files, rtracklayer::import)
  # all_peaks <- GenomicRanges::reduce(unlist(as(dbp_peaks, "GRangesList")))
  # # Make a matrix
  # peak_exists <- matrix(NA, nrow = length(all_peaks), ncol = length(dbp_files))
  # for(k in 1:length(dbp_peaks)) {
  #   peak_exists[,k] <- as.numeric(countOverlaps(all_peaks, dbp_peaks[[k]]) > 0)
  # }
  # # Count overlaps is equivalent to the following
  # # countOverlaps(all_peaks, dbp_peaks[[1]])
  # fo <- findOverlaps(all_peaks, dbp_peaks[[1]])
  # head(fo@from, n = 10)
  # head(countOverlaps(all_peaks, dbp_peaks[[1]]), n = 15)
  # table(rowSums(peak_exists))
  # 
  # # filter to consensus requiring peaks to be in all replicates
  # dbp_consensus <- all_peaks[rowSums(peak_exists) == ncol(peak_exists)]
  # # Required only two replicates
  ...
  for(j in 2:length(dbp_files)) {
    # See the read peaks function to know what subfunctions are called.
    replicate_peaks <- rtracklayer::import(dbp_files[[j]])
    suppressWarnings(pl_ov <- findOverlaps(dbp_consensus, replicate_peaks))
    pl1 <- dbp_consensus[unique(pl_ov@from)]
    pl2 <- replicate_peaks[unique(pl_ov@to)]
    suppressWarnings(dbp_consensus <- GenomicRanges::reduce(union(pl1, pl2)))
    # Collect some info about this replicate file
    peak_info <- bind_rows(peak_info,
                           tibble(dbp = dbp,
                                  file = dbp_files[[j]],
  ...
  # Now we can get the length of the consensus peaks
  peak_info[peak_info$dbp == dbp, "nconsensus"] <- length(dbp_consensus)
  
  # Add the consensus peaks to the consensus peak list
  consensus_peaks <- c(consensus_peaks, list(dbp_consensus))
  names(consensus_peaks)[[i]] <- dbp
}
