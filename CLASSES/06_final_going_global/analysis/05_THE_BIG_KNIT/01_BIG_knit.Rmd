...
promoter_peak_occurence_matrix <- count_peaks_per_feature(lncrna_mrna_promoters, filtered_consensus_list, type = "occurrence")

# saving
write.table(promoter_peak_occurence_matrix, "results/promoter_peak_occurrence_matrix.tsv")
...
# creating promoter peak occurence matrix > DF
...
peak_occurence_df <- data.frame("gene_id" = colnames(promoter_peak_occurence_matrix),
                                "gene_name" = lncrna_mrna_promoters$gene_name,
                                "gene_type" = lncrna_mrna_promoters$gene_type,
                                "chr" = lncrna_mrna_promoters@seqnames,   
                                "1kb_up_tss_start" = lncrna_mrna_promoters@ranges@start,
                                "strand" = lncrna_mrna_promoters@strand,
                                "number_of_dbp" = colSums(promoter_peak_occurence_matrix))
...
pivot_longer(2:ncol(.), names_to = "gene_id", values_to = "occurence") %>%
  filter(occurrence == 1) %>%
...
```
Result: 
(i) Most DBPs show similar binding profiles between lncRNA/mRNA promoters
(ii) a few interesting ones EZH2 is more bound over TSS of lncRNA promoters
(iii) H2AFZ is the most depleted over TSS
(iv) H3K4me2 is more enriched over lncRNA TSS
(v) H3K4me1 is slightly more over lncRNA promoters
(vi) ZNF460 has different pattern between mRNA and lncRNA promoters


# DBP Binding versus RNA expression output
Now we are going to explore how binding to a promoter relates to gene
expression output. We will use RNAseq data from ENCODE in HEPG2.
The data is RNA from differnet fractions -- it was run throguh 
NF_CORE RNAseq pipeline v1.4.2
...
First we want to load in our final_samplesheet from 01_DESEQ_counts.Rmd
...
```
result: There appears to be a bimodal distribution of binding at promoters.
those with less than 100dbps and those with more than 250
...
```
RESULT: 
(i) looks very similar to total RNA binding versus expression
...
```
Result:
(i) This yet again confirms lncRNAs have lower expression levels than mRNAs.
(ii) In the nuclear fraction it shift's to closer.
(ii) lot's of mRNA with nuclear expression -- that seems odd
...
```
Result:
(i) There are 2,360 reservoirs in this largert data set in HEPG2
...
```
Result:
(i) There are 345 reservoirs in both K562 and HEPG2
(ii) There are 80 Hepg2 reservoirs that overlap "conservative" k562 reservoirs
