...
```
# TODO take out wget and use transfer rename to samples.txt
...
2) right click on download TSV button 
3) copy link address.
...
now we will use wget to grab this report file 
or you can download and file transfer.

```{bash}

# go to encode data folder

wget -O samples.txt https://www.encodeproject.org/report.tsv?type=Experiment&status=released&perturbed=false&biosample_ontology.classification=cell+line&target.label=HDAC2&target.label=HDAC1&biosample_ontology.term_name=HepG2&assay_title=TF+ChIP-seq&biosample_ontology.classification=cell+line&files.file_type=fastq

# here we are using wget with the "-O" flag to say the output file name
# samples.txt
...
Now let's parse the sample information using a custom function in our util
...
# First establish the API mode of communication
```{r encode API syntax}
...
# second create a function to make URLs to get the info we want
...
But first we need to check on the fastq files md5sum test next (02_fastq_checking)
