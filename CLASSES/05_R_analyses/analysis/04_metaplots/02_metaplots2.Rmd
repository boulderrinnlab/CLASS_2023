...
# Pivot wider into a matrix for correlation analysis of patterns
...
# Strategy 1: lncrna and mrna metaplots individually
...
# creating promoters just in case (note indexing [ ]):
...
# kinda grainy how many peaks?
length(consensus_peaks[["HDAC2"]])
...
Result: They look similar !


# STRATEGY 2 : Metaplots seperated by lncRNA and mRNA promoters
In this strategy we run profile TSS for lncrnas and mrna seperately.
...
# Now we need to seperate lncRNA and mRNA promoters
# starting with lncRNAs

# TODO why doesn't this work?
# TODO in the for loop make a new col of DBP name and facet wrap by it?
...
for(i in 1:length(consensus_peaks)) {
  print(names(consensus_peaks)[[i]])
  tmp_df <- profile_tss(consensus_peaks[[i]], lncrna_promoters)
  tmp_df$dbp <- names(consensus_peaks)[[i]]
  metaplot_df <- bind_rows(lncrna_metaplot_df, tmp_df)
...
for(i in 1:length(consensus_peaks)) {
  print(names(consensus_peaks)[[i]])
  tmp_df <- profile_tss(consensus_peaks[[i]], mrna_promoters)
  tmp_df$dbp <- names(consensus_peaks)[[i]]
  metaplot_df <- bind_rows(mrna_metaplot_df, tmp_df)
