...
peak_occurence_df <- read_csv("../01_peak_features/results/peak_occurence_df.csv")
...
###### 5 min to generate heatmap - maybe skip until later #####
...
big_dat <- read_csv("/scratch/Shares/rinnclass/CLASS_2023/JR/CLASS_2023/CLASSES/06_final_going_global/analysis/01_create_consensus_peaks/results/peak_occurence_dataframe.csv")

hist(big_dat$number_of_dbp)

# cool result
ggplot(big_dat, aes(x = number_of_dbp)) +
...
# cool result with high binding no expression
...
# Merging in TPM we just made
big_promoter_features_df <- merge(big_dat, tpm)

ggplot(big_promoter_features_df, 
            aes(y = log2(tpm_homo_sapiens_hepg2 + 0.001), x = number_of_dbp, color = gene_type)) + 
  
#geom_point(data = big_promoter_features_df %>% filter(tpm_homo_sapiens_hepg2 > 0.001),
            #shape = 17, alpha = 0.7) +
...
#### Possible for final resutls --SKIP ####
